...
function cleanEnv() {
  return Object.fromEntries(
    Object.entries(process.env).filter(
      ([key]) => !key.startsWith("BUN_BURNER_"),
    ),
  );
}
...
test("construction opens no listener even with sync enabled; failed start is retryable; lifecycle serializes", async () => {
  ...
  try {
    ...
    expect(
      await (await fetch(`http://127.0.0.1:${reserved.port}`)).text(),
    ).toBe("Bun Burner");
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      probe.listen(reserved.port, "127.0.0.1", resolve);
    });
  ...
  try {
    for (const [flag, expected] of [
      ...
      const child = Bun.spawnSync(
        [process.execPath, path, flag!, "--config", "missing.jsonc"],
        { cwd, env: { ...cleanEnv(), BUN_BURNER_PORT: "bad" } },
      );
      expect(child.exitCode).toBe(0);
      expect(child.stdout.toString()).toContain(expected!);
    }
    const child = Bun.spawnSync([process.execPath, path, "--port", "bad"], {
      cwd,
      env: cleanEnv(),
    });
